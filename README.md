...
- `get_replies(note_id: int)`: List replies under a Note, returning full reply fields (`content`, `user`, `entity`, timestamps, `attachments`). Replaces the previous `get_all_replies_with_note_id`, which only returned ids.
